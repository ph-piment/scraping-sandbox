...
func (repo *edinetRepository) FetchBigHolders(ctx context.Context, date string) ([]domain.EdinetDoc, error) {
	...
	for _, doc := range edinetResp.Results {
		if doc.IsLargeShareholdingReport() {
			// fmt.Println("DocDescription", doc.DocDescription)
			// fmt.Println("EdinetCodeName", edinetCodeEntriesMap[doc.EdinetCode].Name)
			// fmt.Println("IssuerEdinetCodeName", edinetCodeEntriesMap[doc.IssuerEdinetCode].Name)
